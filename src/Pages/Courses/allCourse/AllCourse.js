...
const AllCourse = () => {
    ...
    return (
        ...
        <div className='flex'>
            ...
            <div className='bg-gray-800 text-white px-5 lg:px-10 drop-shadow-2xl'>
                {
                    courses.map(courses =>
                        <button key={courses._id} className="  block py-3 my-1 rounded-sm mx-auto">
                            <Link key={courses._id} className='right-side-textss text-decoration-none text-white' to={`/courses/${courses?._id}`}>
                                {courses?.name}</Link>
                        </button>
